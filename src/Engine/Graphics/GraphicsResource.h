...
{
public:
	[[nodiscard]] static const std::vector<VertexAttribute> GetAttribs();

	glm::vec3 positions;  // Vertex position (XYZ - 3 components per vertex) (shader-location = 0)
	glm::vec3 normals;    // Vertex normals (XYZ - 3 components per vertex) (shader-location = 1)
	glm::vec4 tangents;   // Vertex tangents (XYZW - 4 components per vertex) (shader-location = 2)
	glm::vec4 colors;     // Vertex colors (XYZW - 4 components per vertex) (shader-location = 3)
	glm::vec2 texCoords;  // Vertex texture coordinates (UV - 2 components per vertex) (shader-location = 4)
	glm::vec2 texCoords2; // Vertex texture second coordinates (UV - 2 components per vertex) (shader-location = 5)
};
...
public:
	...
	std::vector<uint32_t> indices; // Vertex indices (in case vertex data comes indexed)
...
public:
	...
	NewBoneInfo* bones = nullptr;        // Bones information (skeleton)
	TempTransform* bindPose = nullptr;    // Bones base transformation (pose)
};
