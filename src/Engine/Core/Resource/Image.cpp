...
bool Image::BeginLoad(Stream& source)
{
	...
	if (fileID == "DDS ")
	{
		...
		size = glm::ivec3(ddsd.dwWidth, ddsd.dwHeight, glm::max((int)ddsd.dwDepth, 1));
	...
	else if (fileID == "\253KTX")
	{
		...
		data = new unsigned char[dataSize];
		size = glm::ivec3(imageWidth, imageHeight, 1);
		numLevels = mipmaps;
	...
	else
	{
		...
		SetSize(glm::ivec3(imageWidth, imageHeight, imageDepth), componentsToFormat[imageComponents]);
...
bool Image::GenerateMipImage(Image& dest) const
{
	...
	glm::ivec3 sizeOut(std::max(size.x / 2, 1), std::max(size.y / 2, 1), std::max(size.z / 2, 1));
...
ImageLevel Image::Level(size_t index) const
{
	...
	for (;;)
	{
		level.size = glm::ivec3(std::max(size.x >> i, 1), std::max(size.y >> i, 1), std::max(size.z >> i, 1));
