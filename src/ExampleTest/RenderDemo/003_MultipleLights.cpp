...
bool _003MultipleLights::Create()
{
	...
	{
		{.location = 0, .size = 3, .normalized = false, .stride = sizeof(float) * 8, .offset = (void*)0},
		{.location = 1, .size = 3, .normalized = false, .stride = sizeof(float) * 8, .offset = (void*)(3 * sizeof(float))},
		{.location = 2, .size = 2, .normalized = false, .stride = sizeof(float) * 8, .offset = (void*)(6 * sizeof(float))},
	};

	m_geom = renderSystem.CreateGeometryBuffer(BufferUsage::StaticDraw, static_cast<unsigned>(Countof(vertices)) / 8, static_cast<unsigned>(sizeof(float)) * 8, vertices, formatVertex);

	m_diffuse = renderSystem.CreateTexture2D("../ExampleData/textures/container2.png");
	m_specular = renderSystem.CreateTexture2D("../ExampleData/textures/container2_specular.png");
...
void _003MultipleLights::Render()
{
	...
	renderSystem.SetUniform("uMaterial.specular", 1);

	renderSystem.Bind(m_diffuse, 0);
	renderSystem.Bind(m_specular, 1);
