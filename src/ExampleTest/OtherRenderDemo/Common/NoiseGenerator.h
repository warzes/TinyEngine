...
public:
	...
	void setup(uint32_t noise_start_seg, uint32_t noise_levels, float noise_start_factor, float noise_factor, Noise_Function_select noise) {
	...
	std::vector<std::vector<float> > get_2D_noise(unsigned int size_2d_x, unsigned int size_2d_y, float min_x, float max_x, float min_y, float max_y) {
		...
		for (uint32_t i = 0; i < size_2d_y; i++) {
			ret_vec[i].resize(size_2d_x);
			for (uint32_t j = 0; j < size_2d_x; j++) {
...
protected:
	...
	float function_recurs_noise(float x, float y) {
		...
		for (uint32_t i = 0; i < noise_levels; i++) {
